...
class CarlaScoomatic(object):

    """
    Handles the spawning of the scoomatic and its sensors

    Derive from this class and implement method sensors()
    """

    def __init__(self):
        rospy.init_node('scoomatic', anonymous=True)
	...
	self.control = carla.ScoomaticControl()
        ...
        if spawn_point_param and rospy.get_param('~spawn_scoomatic'):
    ...
    def restart(self):
        ...
        # Spawn the scoomatic.
        if not rospy.get_param('~spawn_scoomatic'):
    ...
    def setup_sensors(self, sensors):
        """
        Create the sensors defined by the user and attach them to the scoomatic
    ...
    def run(self):
        ...
        rospy.loginfo("CARLA world available. Spawn scoomatic...")
...
def main():
    ...
    main function
    """
    scoomatic = CarlaScoomatic()
    try:
        scoomatic.run()
    finally:
        if scoomatic is not None:
            scoomatic.destroy()
